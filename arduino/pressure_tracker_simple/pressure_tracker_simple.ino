/*
 * PURE DIGITAL PRESSURE TRACKER - SIMPLE VENT DEFLATION
 * Same as pressure_tracker.ino, but WITHOUT the reverse-PFM descent:
 * when the pressure is above target the valve is simply held open
 * until the deadband is reached. No burst/settle modulation.
 *
 * Safety features:
...
void loop() {
  ...
  // The 3-field form "target,max,min" is still accepted for host
  // compatibility; the two PFM timings are parsed and ignored here.
  ...
  if (target_kPa <= 0.1) {
    ...
    if (error > DEADBAND_KPA) {
      ...
      // DEFLATE: valve simply held open until back inside the deadband.
      setPump(false);
      setValve(true, now);
      state_name = "DEFLATE";
    }
