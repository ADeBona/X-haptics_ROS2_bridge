...
If `kinova_haptic_bridge` (or anything else) shows up, the bridge is racing `pad_logger.py` for the same serial line and your `U`/`D` keys land nowhere. Stop it per §11, confirm `lsof` prints nothing, then reconnect.
